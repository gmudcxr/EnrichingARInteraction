...
public partial class MapGraph
{
    ...
    public MapGraph(Voronoi voronoi, float snapDistance, Dictionary<Vector2, string> siteTagMap)
    {
        this._voronoi = voronoi;
        
        CreateFromVoronoi(voronoi, siteTagMap);
    ...
    private void CreateFromVoronoi(Voronoi voronoi, Dictionary<Vector2, string> siteTagMap)
    {
        ...
        foreach (var site in voronoi.SiteCoords())
        {
            ...
            // add tag
            var tag = siteTagMap[site];
            var currentNode = new MapNode { centerPoint = center, tag = tag};
    ...
    //NOTH: changed to public to have the heightmap generated later
    public void UpdateHeights(HeightMapGenerator.HeightMap heightmap)
    {
    ...
    private static Vector3 UpdateHeight(HeightMapGenerator.HeightMap heightmap, Vector3 oldPosition)
    {
    ...
    public List<LineSegment> GetBoundriesForSite(Voronoi voronoi, Vector2 site)
    {
    ...
    public bool SiteOwnsBoundary(Voronoi voronoi, Vector2 site, LineSegment line)
    {
        var boundries = voronoi.VoronoiBoundaryForSite(site);

        foreach (var boundary in boundries)
        {
            if((boundary.p0.Value.Equals(line.p0.Value) && boundary.p1.Value.Equals(line.p1.Value)) ||
               (boundary.p1.Value.Equals(line.p0.Value) && boundary.p0.Value.Equals(line.p1.Value)))
                return true;
        }
